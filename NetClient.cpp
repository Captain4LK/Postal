   ...
   short sBandwidth)                            // In:  Joiner's Net::Bandwidth
{
   ...
   // set a timer so that if it takes too S32, we'll give up.
      ...
      else
      {
         ...
         case NetMsg::DROPPED:
         {
            ...
            else
            {
               ...
               // kick off the beginning of a S32 sequence...
         ...
         case NetMsg::START_GAME:
            ...
            m_lFrameTime = (S32)pmsg->msg.startGame.sFrameTime;
   ...
   const char* pszText)                         // In:  Text to send
{
   if (m_state == Joined)
   {
      ...
      // Copy text, truncating in case the specified text is too S32
   ...
   if (m_bPlaying)
   {
      ...
      for (Net::ID id = 0; id < Net::MaxNumIDs; id++)
      {
         if (m_aPeers[id].m_state == CPeer::Joined)
         {
            ...
            S32 temp = m_aPeers[id].m_netinput.GetFrameTime(m_seqFrame); // *SPA
            lFrameTime += temp; // *SPA
            sCount++;
...
////////////////////////////////////////////////////////////////////////////////
// This is used to see if any peers have not sent data for too S32 of a period
//
   ...
   if (m_bPlaying)
   {
      for (Net::ID id = 0; id < Net::MaxNumIDs; id++)
      {
         if (m_aPeers[id].m_state == CPeer::Joined)
         {
            if (id != m_id)
            {
               S32 lElapsedTime = rspGetMilliseconds() - m_aPeers[id].m_lLastReceiveTime;
