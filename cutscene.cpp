...
inline void MySeedRandom(S32 seed)
{
   ms_lRandom = seed;
}

inline S32 MyRandom(void)
{
                     ...
                     )
{
   ...
   S32 lStartTime = rspGetMilliseconds();
   S32 lCurrentTime = -1;

   S32 lStartFadeTime = lMilliLen - lFadeTime;    // lCurrentTime is relative to lStartTime

   // Attempt to do a fade out while swirling....
   while (((lCurrentTime = (rspGetMilliseconds() - lStartTime)) < lMilliLen) && !(rspGetQuitStatus()))
   {
      S32 lCurrentFadeTime = lCurrentTime - lStartFadeTime;
      //==============================================================================
...
double m_dRadA;
S32 m_lCycleTimeX;         // in milliseconds (Min, Max, Min)
S32 m_lCycleTimeY;         // in milliseconds (Min, Max, Min)
S32 m_lCycleTimeA;         //
S32 m_lTimeSpin;
S32 m_lBaseTime;
...
short   MakeFrame(SampleMasterID* psmid)
{
   ...
   S32 lDeltaTime = rspGetMilliseconds() - m_lBaseTime;
...
short Configure(      //RImage* pimDst,RImage* pimSrc,RMultiAlpha* pX,
   S32 lTimeSpin,
   short sMinX, short sMaxX, S32 lTimeX,
   short sMinY, short sMaxY, S32 lTimeY,
   double dMinA, double dMaxA, S32 lTimeA,
...
static void CutScene_RFileCallback(S32 lBytes);
   ...
   short sBorderY)
{
   ...
   else
   {
      ...
      {
         TRACE("CutScene(): Bg file name/path too S32: '%s'!\n", szText);
         strcpy(szText, DEFAULT_BG);
      }
   }
   ...
   else
   {
      ...
      {
         TRACE("CutScene(): Title too S32: '%s'!\n", ms_pCut->m_szTitle);
         strcpy(ms_pCut->m_szTitle, DEFAULT_TITLE);
      }
   }
   ...
   else
   {
      ...
      {
         TRACE("CutScene(): Music filename too S32: '%s'!\n", ms_pCut->m_szMusic);
         strcpy(ms_pCut->m_szMusic, DEFAULT_MUSIC);
      }
   }
