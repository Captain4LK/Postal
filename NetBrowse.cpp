   ...
   Hosts* phostsRemoved)                        // I/O:  List of hosts that were removed
{
   ...
   if (serr == 0)
   {
          ...
          (buf[3] == Net::BroadcastMagic3))
      {
         ...
         // that sent it, so as S32 as the encoding and decoding of the bytes
         ...
         host.m_lMagic =
            ((S32)buf[4] & 0x000000ff) +
            (((S32)buf[5] <<  8) & 0x0000ff00) +
            (((S32)buf[6] << 16) & 0x00ff0000) +
            (((S32)buf[7] << 24) & 0xff000000);
   ...
   // Check for hosts that haven't been heard from in too S32 a time,
