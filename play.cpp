...
S32 m_lTimeBomb;                                   // Time when bomb explodes
   ...
   RInputEvent* pie)                                  // I/O: Input event
{
   if (pinfo->IsMP())
   {
      ...
      // user decided he had waited S32 enough).
      ...
      // that quits the game if it takes too S32 to do it the "right" way.
...
// far from an easy fix.
      ...
      // loop that loops as S32 until we've slowed to the proper frame rate.
      //
...
//------------------------------------------------------------------------------
private:
S32 m_lLastFrameTime;
S32 m_lSumFrameTimes;
S32 m_lNumFrames;
   ...
   CPlayInfo* pinfo)                                  // I/O: Play info
{
   if (!pinfo->m_bBadRealmMP)
   {
      ...
      if (pinfo->IsMP())
      {
         ...
         {
            S32 lCurrSeqTime = rspGetMilliseconds();
         ...
         if (GetInputMode() != INPUT_MODE_LIVE)
         {
            ...
            if (pdudeLocal != NULL)
            {
               if (pdudeLocal->IsDead() == true)
               {
                  ...
                  // If he's been dead S32 enough, end the game
      ...
      if (pfileDemoModeDebugMovie->IsOpen())
      {
         ...
         else
         {
            ...
            if (im.Load(pfileDemoModeDebugMovie) == 0)
            {
               ...
               U8* pDstLine = g_pimScreenBuf->m_pData + ((S32)FILM_Y * g_pimScreenBuf->m_lPitch) + (S32)FILM_X;
   ...
   CPlayInfo* pinfo)                                  // I/O: Play info
{
   ...
   S32 lBlackTime = rspGetMilliseconds();
         ...
         if (!sResult && bGameReady)
         {
            ...
            if (sResult == 0)
            {
               ...
               do	{
                  ...
                  if (!sResult)
                  {
                     ...
                     if (!sResult && bRealmReady)
                     {
                        ...
                        if (pclient)
                        {
                           ...
                           info.Realm()->m_lScoreInitialTime = info.Realm()->m_lScoreTimeDisplay = (S32)sTimeLimit * (S32)60000;
                  ...
                  const S32 endRealmMS = rspGetMilliseconds();
                  const S32 timePlayedMS = ((startRealmMS > 0) && (endRealmMS > 0) && (endRealmMS > startRealmMS)) ? endRealmMS - startRealmMS : -1;
                  const S32 newPlaythroughMS = playthroughMS + timePlayedMS;
   ...
   short sMaxTitleLen /*= NULL*/)               // In:  Max length of returned title, including terminating null
{
   ...
   if (sResult == 0)
   {
      ...
      {
         // File name too S32 (and can't be truncated)
         sResult = -1;
         TRACE("Play_GetRealmInfo(): Realm file name/path too S32!\n");
