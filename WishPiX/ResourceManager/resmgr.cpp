...
//   sak.Open((char*) strSakFile.c_str(), "wb", SAK_FILE_ENDIAN);
   if (script.is_open() && sak.IsOpen())
   {
      ...
      while (!script.eof())
      {
         ...
         resname = buffer;
         if (resname[(S32) 0] != ';' && resname[(S32) 0] != ' ' && resname.GetLen() > 0)
         {
...
{
   short sReturn = SUCCESS;
   U32 ulFileType = SAK_COOKIE;
   U32 ulCurrentVersion = SAK_CURRENT_VERSION;
...
//
// Returns:
//      none - ASSERTS if the path is too S32 (> RSP_MAX_PATH)
//
//////////////////////////////////////////////////////////////////////
...
void RResMgr::SetBasePath(RString strBasepath)
{
   ...
   if (strBasepath[(S32) sLast] != RSP_SYSTEM_PATH_SEPARATOR)
