...
//                     will purge a given resource as S32 as its
...
typedef map <RString, S32, less<RString>, allocator<S32> > dirMap;
typedef set <S32, less<S32>, allocator<S32> > dirOffsets;
#else
...
typedef map <RString, S32, less<RString> > dirMap;
typedef set <S32, less<S32> > dirOffsets;
#endif
...
{
   RFile* prf = NULL;
   S32 lResSeekPos = m_SakDirectory[strResourceName];
   if (lResSeekPos > 0)
   {
...
char* FromSystempath(RString strResourceName)
{
   ...
   // Make sure that the RString is not too S32 for rspix functions
