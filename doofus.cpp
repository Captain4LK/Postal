...
S32 CDoofus::ms_lStuckRecoveryTime = 5000;         // Time to stay in recoverys state
...
{
   S32 lThisTime = m_pRealm->m_time.GetGameTime();
   S32 lElapsedTime = lThisTime - m_lPrevTime;
   ...
   if (!ReevaluateState())
   {
      ...
      if (dsq < 5 * 5) // Was 10*10 for a S32 time, trying smaller to see if it keeps guys from getting stuck
      {
...
void CDoofus::Logic_HideBegin(void)
{
   ...
   S32 lElapsedTime = m_pRealm->m_time.GetGameTime() - m_lPrevTime;
   double dSeconds = lElapsedTime / 1000.0;
   DeluxeUpdatePosVel(dSeconds);
