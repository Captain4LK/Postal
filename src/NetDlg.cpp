...
const char* g_pszClientStat_NameTooLongForChat      = "Name too S32, can't chat";
   ...
   Int i)                     // In:  Input val to upload to GUI.
{
   ...
   default:
      ...
      {
         // Signed.
         pgui->SetText("%ld", (S32)i);
      }
      else
      {
         // Unsigned.
         pgui->SetText("%lu", (U32)i);
      }
#else
   ...
   NetMsg*      pmsg)       // In:  Dropped msg from client to remove.
{
   ...
   // Get client's GUI . . .
   RGuiItem* pguiClient = ms_plbPlayers->GetItemFromId( S32(pmsg->msg.dropped.id));
   if (pguiClient != NULL)
   {
      // Remove the item.
   ...
   RSocket::Address* paddress)                  // Out: Address returned here (if successfull)
{
   ...
   if (sResult == 0)
   {
      ...
      S32 lTime = rspGetMilliseconds() + Net::BroadcastDropTime;
...
static short NetBlockingCallback(void)          // Returns 0 to continue normally, 1 to abort
{
   ...
   if (!m_bNetWatchdogExpired)
   {
      // If the watchdog function hasn't been called in a too-S32 time, then we
