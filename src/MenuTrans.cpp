...
static S32 m_lTotalTime;
static S32 m_lBaseTime;
...
extern void StartMenuTrans(
   S32 lTotalTime)                              // In:  Effect time (in ms) must be >= 0
   {
...
// IF you comment this back in, remember to comment in the unlock as well!
   ...
   if (rspLockBuffer() == 0)
      ...
      short sWidth2;
      S32 lNextRow = lPitch - (S32)sWidth;
      ...
      if ((sHeight > 0) && (sWidth > 0))
         ...
         do {
            sWidth2 = sWidth;
            do   {
               *pBuf = *(aucMap + (S32)*pBuf);   // may be faster than aucMap[*pBuf]
               pBuf++;
               } while (--sWidth2);
            pBuf += lNextRow;
            } while (--sHeight);
