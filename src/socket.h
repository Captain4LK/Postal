...
public:
   ...
   S32 lAddressLen;                                // Actual address length (depends on protocol, always <= MaxAddressSize)
...
virtual short Send(                                         // Returns 0 if data was sent
   ...
   S32 *plActualBytes)                                      // Out: Acutal number of bytes sent
= 0;
...
virtual short Receive(                                      // Returns 0 if data was received
   ...
   S32* plActualBytes)                                      // Out: Actual number of bytes received into buffer
= 0;

// RecieveFrom - receive data from given address
virtual short ReceiveFrom(                                  // Returns 0 if data was received
   void* pBuf,                                              // In:  Pointer to data buffer
   S32 lMaxBytes,                                        // In:  Maximum bytes that can fit in buffer
   S32* plActualBytes,                                      // Out:  Actual number of bytes received into buffer
...
virtual S32 CheckReceivableBytes(void) = 0;
