...
short CSmashatorium::CollideCyl(CSmash* pSmashee, RSphere* pSphere)   // sphere of Smasher
{
   ...
   S32 lCylR = pSmashee->m_sphere.sphere.lRadius / 3;    // go with half the sphere radius
...
short CSmashatorium::CollideCyl(CSmash* pSmashee, R3DLine* pLine)   // sphere of Smasher
{
   ...
   // it's a dude !  see if the cylinder collides:
   S32 lCylR = pSmashee->m_sphere.sphere.lRadius / 3;    // go with half the sphere radius
   S32 lOldR = pSmashee->m_sphere.sphere.lRadius;
...
short CSmashatorium::Alloc(short sWorldW, short sWorldH, short sTileW, short sTileH)
{
   ...
   m_sClipW = S32(m_sGridW) * sTileW;
   m_sClipH = S32(m_sGridH) * sTileH;

   // Note that we must add 2 tile lengths to each access line:
   m_pGrid = new CSmashatoriumList[S32(m_sGridW) * m_sGridH];
   ...
   for (g = 0, p = 0, i = 0 ; i < m_sClipH; i += sTileH, g++)
   {
      for (j = 0; j < sTileH; j++, p++)
      {
         ...
         m_ppslAccessY[p] = m_pGrid + S32(m_sGridW) * g;
      }
   }
   ...
   CSmash** ppSmashee)
{
   ...
   S32 lClosestDist2 = 2000000000; // a large number
   S32 lCurDist2;

   S32 lSmasherX = pSphere->X;
   S32 lSmasherY = pSphere->Z;
   ...
   CSmash*   pSmasher)                  // Out: Smash that should be excluded from search.
{
   ...
   S32 lDelX = lRight - lLeft;
   S32 lDelY = lRightY - lLeftY;
   S32 lDet = lDelX * lRightY - lDelY * lRight;
   S32 lDetY = lDelY * lRight - lDelX * lRightY;   // for x-major line form
   ...
   S32 lClipLeft = lLeft;  // = MAX(0,lLeft);
   S32 lClipRight = lRight;  // = MIN(m_sWorldW - 1,lRight);
   S32 lClipLeftY = lLeftY;
   S32 lClipRightY = lRightY;
   ...
   lGridLeft = (S32)m_psClipX[lClipLeft]; // These represent pts BETWEEN grid squares
   lGridRight = 1 + (S32)m_psClipX[lClipRight];
   ...
   {                                         // Note that the vertical case has
      ...
      if (lDelY > 0) // lClipLeftY < lClipRightY:
      {
         if (lClipLeftY < 0)
         {
            ...
            lGridLeft = (S32)m_psClipX[lClipLeft];
            sClippingY = true;
         }
...
#define MAX_GRID_W 1024 //************************************ NEED TO DEAL WITH THIS!
   ...
   S32 alPointsY[MAX_GRID_W + 1];
   short i, x;

   // Get end points:
   alPointsY[lGridLeft] = (S32)m_psClipY[lClipLeftY];
   alPointsY[lGridRight] = (S32)m_psClipY[lClipRightY];
   ...
   {     // not vertical strip:
      ...
      for (i = lGridLeft + 1; i < lGridRight; i++, x += m_sTileW)
      {
         ...
         alPointsY[i] = (S32)m_psClipY[alPointsY[i]]; // convert to grid coordinates
      }
   }
