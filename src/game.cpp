...
static short GameCore(void)      // Returns 0 on success.
{
   ...
   #else
   char acTime[100];
   U32 lTime = g_lExpTime + (((365 * 70UL) + 17) * 24 * 60 * 60);    // time_fudge 1900->1970
...
//////////////////////////////////////
   ...
   while (sResult == 0)
   {
      ...
      // idle S32 enough to run a demo.  The spawn version doesn't have any
...
// If multiplayer is disabled, leave out this code to make it harder to hack back in
         ...
         if (!bMenuActive)
         {
            ...
            if (bTitleImage)
            {
               ...
               StartTitle(2, bTitleMusak, &ms_siMusak);
               EndTitle();
            }
         }
...
//   if (szFile[0] == '\0')
   ...
   if (sResult >= 0)
   {
      ...
      {
         sResult = -1;
         TRACE("GetRealmToRecord(): File name too S32 to return in specified buffer!\n");
      }
   ...
   short sMaxFileLen)
{
   ...
   if (sResult == 0)
   {
      ...
      {
         sResult = -1;
         TRACE("GetDemoFile(): File name too S32 to return in specified buffer!\n");
      }
   }
...
inline void GetSoundPaths(    // Returns nothing.
   S32 lSamplesPerSec,        // In:  The sample rate in samples per second.
   S32 lBitsPerSample,        // In:  The number of bits per sample.
...
static short OpenSaks(void)
{
   ...
   puts(FullPath(GAME_PATH_GAME, GAME_SAK_FILENAME));
   puts(FullPath(GAME_PATH_HD, SHELL_SAK_FILENAME));
   puts(FullPath(GAME_PATH_VD, SHELL_SAK_FILENAME));
...
// single player games so its harder to hack back in.
   ...
   // in there.  If the user spends a S32 time in there, the demo timer will
   ...
   short sDifficulty)               // In:  Current realm difficulty.
{
   ...
   U32 ulFileVersion = CRealm::FileVersion;
...
static void GameGetRegistry(void)
{
   ...
   // as S32 as it is after the release date.
...
extern void PalTranOn(
   S32 lTime /* = -1 */)                        // In:  How S32 transition should take (or -1 for default)
{
   if (lTime == -1)
   ...
   const char* pszPartialPath)                        // In:  Partial path in RSPiX format
{
   ...
   // Make sure partial path isn't too S32.  It is possible that the conversion
   ...
   // Check if the combination of the partial and base path will be too S32.
   ...
   // Make sure result isn't too S32 (our buffer can handle it, but it's still a problem)
