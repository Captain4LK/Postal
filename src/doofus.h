...
S32 m_lAlignTimer;                     // Recheck position to bouy every so often
S32 m_lEvalTimer;                      // Reevaluate state every so often
S32 m_lShotTimeout;                    // Only do Shot animation every so often
...
S32 m_lIdleTimer;                      // Timer for idle animations.
bool m_bAnimUp;                        // Run animation up or down for idle animation. (crouch)

S32 m_lSampleTimeIsPlaying;                  // Expected time for this sample
...
S32 m_lGuardTimeout;                         // Tunable personatorium value with doofus default
S32 m_lShootTimeout;                         // Tunable time between shots - based on difficulty level
S32 m_lRunShootInterval;                     // Tunable personatorium value with doofus default.
...
static S32 ms_lDefaultAlignTime;      // How often to recalibrate angle to bouy
static S32 ms_lGuardTimeoutMin;        // How often to check for CDudes proximity
static S32 ms_lGuardTimeoutInc;        // Amount of time between for each level of difficulty
static S32 ms_lShootTimeoutMin;        // How often to wait between shots, min
static S32 ms_lShootTimeoutInc;        // Variance between shot times based on difficulty level
static S32 ms_lDetectionRadius;        // Radius of detection sphere
static S32 ms_lRunShootInterval;      // Time to run between shooting
static S32 ms_lReseekTime;          // Time to go before seeking the dude's position again
static S32 ms_lShotTimeout;            // Time to go before doing full shot anim when shot
                                       // this will give him time to escape.
static S32 ms_lAvoidRadius;            // Radius of fire avoidance smash
static S32 ms_lYellRadius;          // Radius of alerting smash
static S32 ms_lHelpTimeout;            // Time to react to a call for help.
static S32 ms_lDelayShootTimeout;      //time before shooting
static S32 ms_lHelpingTimeout;         // time before shooting when helping
static S32 ms_lStuckRecoveryTime;      //time to allow recovery from stuck position
