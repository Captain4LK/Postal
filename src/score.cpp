...
static S32 ms_lScoreMaxTimeOut;     // Optional score timeout (max time spent
                                    // on score screen).
                        ...
                        short sDstX, short sDstY, CHood* pHood)
{
   ...
   if (lCurTime > g_scoreboard.m_lLastScoreDrawTime + SCORE_UPDATE_INTERVAL)
   {
      ...
      if (pHood)
         rspBlit(pHood->m_pimTopBar, pim, 0, 0, sDstX, sDstY,
...
void ScoreDisplayHighScores(  // Returns nothing.
   ...
   S32 lMaxTimeOut)           // In:  Max time on score screen (quits after this
                              // duration, if not -1).
{
   RGuiItem* pguiRoot;
   ...
   if (pRealm->m_ScoringMode >= CRealm::Timed && pRealm->m_ScoringMode <= CRealm::MPLastManTimedFrag && GetInputMode() != INPUT_MODE_PLAYBACK)
   {
      ...
      if (pRealm->m_flags.bMultiplayer == false)
      {
         ...
         for (sSrcIndex = 0, sDstIndex = 0; sDstIndex < MAX_HIGH_SCORES; sDstIndex++)
         {
            ...
            {
               if (sOpenRes == 0)
                  scores.GetVal((char*) pRealm->m_rsRealmString, szKeyName, (S32) 0, &(alScores[sDstIndex]));
               else
            ...
            {
               if (sOpenRes == 0)
                  scores.GetVal((char*) pRealm->m_rsRealmString, szKeyName, (S32) 3600000, &(alScores[sDstIndex]));
               else
                  alScores[sDstIndex] = (S32) 3600000;
