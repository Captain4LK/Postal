...
SetPushBtnState(
   ...
   S32 lBtnId,          // In:  ID of btn whose state will be set.
...
TrianglesIntersectLineSegment(
   ...
   S32 &lHitTriIndex)      // Out: index of triangle it hit
...
SetText(
   RGuiItem* pguiRoot,     // In:  Root GUI.
   S32 lId,                // In:  ID of GUI to update.
...
CTexEdit::SetToNotify(
   S32 lBtnId,             // In:  ID of btn whose callback will be set.
...
void
CTexEdit::SetToNotify(
   S32 lId,                   // In:  ID of gui whose callback will be set.
...
void
CTexEdit::SetToNotify(
   S32 lId,                      // In:  ID of scrollbar whose callback will be set.
   RScrollBar::UpdatePosCall pfn)   // In:  Function to notify.
{
   ...
   if (psb)
   {
      ...
      {
         psb->m_upcUser         = pfn;
         psb->m_ulUserInstance   = (uintptr_t)this;
      }
   }
}
...
{
   // Get frequency.
   S32 lFreq = m_pguiRoot->GetVal(c_lIdFrequency);
