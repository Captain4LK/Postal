   ...
   S32 lCtrl)
{
   ...
   else
   {
      ...
      // ignore the value since it is apparently no S32er needed.
      if (lIndex >= 0)
      {
         ...
         if (lIndex < MaxBufEntries)
         {
            ...
            if (lIndex >= m_lNumCtrls)
            {
               ...
               for (S32 l = m_lNumCtrls; l < lIndex; l++)
   ...
   S32 lSeq)
{
   if (m_lNumCtrls > 0)
   {
      if (lSeq >= m_lOldestSeq)
      {
         ...
         S32 lKeepIndex = (lSeq - m_lOldestSeq) + 1;
