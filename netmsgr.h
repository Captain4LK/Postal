   ...
   RSocket::Address* paddress)
{
   ...
   pBuf->Put((S32)paddress->prototype);
...
typedef struct Login
{
   enum { Size = 9 };
   unsigned char ucType;                              // Message type
   U32 ulMagic;                              // Magic number
   U32 ulVersion;                            // Version number
...
typedef struct InputData
{
   ...
   unsigned char ucType;                              // Message type
   S32 lSize;                                         // Message size (must follow type!)
   ...
   static void Read(NetMsg* pmsg, CBufQ* pBuf)
   {
      ...
      pmsg->msg.inputData.pInputs = (UINPUT*)AllocVar(pmsg, (S32) pmsg->msg.inputData.sNum * sizeof(UINPUT));

      // Get the variable-sized data
      pBuf->Get(pmsg->msg.inputData.pInputs, (S32)(pmsg->msg.inputData.sNum));
   }

   static void Write(NetMsg* pmsg, CBufQ* pBuf)
   {
      // Calculate message size
      pmsg->msg.inputData.lSize = 1 + 4 + 1 + sizeof(Net::SEQ) + 2 + ((S32)(pmsg->msg.inputData.sNum) * sizeof(UINPUT));
      ...
      pBuf->Put(pmsg->msg.inputData.pInputs, (S32)(pmsg->msg.inputData.sNum));
   }
} InputData;
...
typedef struct Ping
{
   enum { Size = 1 + 4 + 4 };
   unsigned char ucType;                              // Message type
   S32 lTimeStamp;                                    // Timestap for this ping
   S32 lLatestPingResult;                             // Latest ping result (round trip time)
...
typedef struct Rand
{
   enum { Size = 1 + 4 + 4 };
   unsigned char ucType;                              // Message type
   S32 lFrame;                                        // Current frame
   S32 lRand;                                         // Current rand()
...
////////////////////////////////////////////////////////////////////////////////
S32 GetMostRecentMsgReceiveTime(void)
{
   return m_lMsgRecvTime;
}

S32 GetMostRecentMsgSentTime(void)
{
