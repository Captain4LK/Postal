...
// does not set the PUSH bit on sends, response delays may result.  It뭩 best
   ...
   if (m_sock != INVALID_SOCKET)
   {
      if (!m_bListening)
      {
         ...
         else
         {
            ...
            S32 lCount = select(0, NULL, NULL, &set, &timeval);
            if (lCount == 0)
            {
               // Create a set containing just this socket
               FD_SET set;
               ...
               ms_funcnum = RSocket::SelectFunc;
               S32 lCount = select(0, NULL, &set, NULL, &timeval);
               if (lCount > 0)
               {
                  // Connected!
                  m_bConnecting = false;
...
short RProtocolBSDIP::Receive(                  // Returns 0 on success, non-zero otherwise
   void* pBuf,                                  // In:  Pointer to data buffer
   S32 lMaxBytes,                            // In:  Maximum number of bytes that fit in the buffer
   S32* plActualBytes)                          // Out: Actual number of bytes recieved into the buffer
{
#if PLATFORM_UNIX
   return(-1);   // !!! FIXME
#else
...
short RProtocolBSDIP::ReceiveFrom(              // Returns 0 on success, non-zero otherwise
   void* pBuf,                                  // In:  Pointer to data buffer
   S32 lMaxBytes,                            // In:  Maximum bytes that can fit in the buffer
   S32* plActualBytes,                          // Out: Actual number of bytes recieved into buffer
...
bool RProtocolBSDIP::CanAcceptWithoutBlocking(void)
{
   ...
   if (m_sock != INVALID_SOCKET)
   {
      if (m_bListening)
      {
         ...
         ms_funcnum = RSocket::SelectFunc;
         S32 lCount = select(0, &set, NULL, NULL, &timeval);
         if (lCount > 0)
            bResult = true;
      }
      else
...
// how S32 this will be true, especially in a multi-threading environment
//////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////
S32 RProtocolBSDIP::CheckReceivableBytes(void)
{
#if PLATFORM_UNIX
   return(0);   // !!! FIXME
#else
   ...
   if (m_sock != INVALID_SOCKET)
   {
      if (!m_bListening)
      {
         ...
         ms_funcnum = RSocket::OtherFunc;
         if (ioctlsocket(m_sock, FIONREAD, (U32*)&lResult) == SOCKET_ERROR)
         {
            lResult = 0;
...
short RProtocolBSDIP::GetMaxDatagramSize(       // Returns zero on success, non-zero otherwise
   S32* plSize)                                 // Out: Maximum datagram size in bytes
{
   short sResult = 0;
   ...
   if (ms_bDidStartup)
   {
      *plSize = (S32)ms_WSAData.iMaxUdpDg;
   }
   else
   {
      sResult = -1;
...
short RProtocolBSDIP::GetMaxSockets(            // Returns 0 if successfull, non-zero otherwise
   S32* plNum)                               // Out: maximum number of sockets
{
   short sResult = 0;
   ...
   if (ms_bDidStartup)
   {
      *plNum = (S32)ms_WSAData.iMaxSockets;
   }
   else
   {
      sResult = -1;
