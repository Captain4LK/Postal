...
short   ConvertToROTBUF(RImage* pImage)
{
   ...
   S32 d4 = (SQR(pImage->m_sWidth - cx - 1) + SQR(pImage->m_sHeight - cy - 1));

   S32 lR = MAX(MAX(d1, d2), MAX(d3, d4)); // distance squared
   lR = (S32)(sqrt(double(lR)) + 0.9999); // actual S32est distance, rounded up
   S32 lC = (S32)(sqrt2 * lR + 1);  // round up
   S32 lNewW = (S32)(lC << 1); // new buffer width a test:

   // Store the rotating window size in this field:
   pImage->m_sWinWidth = pImage->m_sWinHeight = (lR << 1);

   // Find a new pitch
   S32 lNewP = (lNewW + 15) & ~15;
...
typedef   struct   tagLine1
{
   ...
   S32 lNumY;
   S32 lIncX;     // Fractional Increment
   S32 lIncY;
   S32 lDelX;     // Default Integral Increment for slopes > 1 as
   S32 lDelY;     // dictated by the Dst W & H....
   S32 lDen;
   S32 lCurX;     // Current point position
   S32 lCurY;
                     ...
                     S32 x1, S32 y1, S32 x2, S32 y2, S32 d)
{
   ...
   pLine->lDen = d;
   pLine->lNumX = pLine->lNumY = (S32)d >> 1;
   pLine->lPitchX = SGN(lDelX);
