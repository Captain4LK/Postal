...
short RFontOld::Create(char *pszFontName,char *pszFullInputName,
   ...
   for (short i=0;i<256;i++)
      ...
      // Add it in:
      //rspSetConvertToFSPR1((U32)ucInitialTrim,(U32)ucBackCol,sTrimTop,sMaxH,(UCHAR)i);
   ...
   while   (pCurFont)   // loop through cached sizes:
      ...
      for (i=0;i<256;i++)
         ...
         if (pimCurLetter = pCurFont->pLetters[i])
            ...
            ucCheckSum = (UCHAR)0;
            for (S32 i=0;i < (S32)pInfo->m_lSize;i++,pCode++)
               ucCheckSum ^= *pCode;

            fprintf(fp,"%02x\n",(unsigned short) ucCheckSum);
            fprintf(fp,"%ld\n",pInfo->m_lSize);
            for (i=0,pCode = pInfo->m_pCode;i < (S32)pInfo->m_lSize;
                     i++,pCode++)
               fprintf(fp,"%c",*pCode);
            fprintf(fp,"\n");
            }
         ...
         *pCode++ = UCHAR(0xff);
         ...
         if (ucCheck != ucCheckSum)
         {
            TRACE("ERROR: character %02x, parity error\n"
                  "%02x vs %02x\n",
                  (U32)pInfo->m_u16ASCII,
                  (U32)ucCheckSum, (unsigned short)ucCheck);
                       ...
                       short /*sJustFraction*/, short /*sDen*/)
{
   ...
   for (short i = 0; i<sNumChar; i++)
   {
      ...
      */
      //   _rspBlit((U32)m_clrFGD,(U32)m_clrBKD,pimCur,m_pimTarget,
      //      m_sCurX,m_sCurY,sW,sH,0,m_pCurFracY);//,m_pCurFracY,m_pCurFracX);

      /* old...
      rspBlit((U32)m_clrFGD,(U32)m_clrBKD,pimCur,m_pimTarget,
         m_sCurX,m_sCurY,sStretchW,sH,&rCol,0,m_psItalic);
      */
      // New...can't clip...
      rspBlit((U32)m_clrFGD, pimCur, m_pimTarget,
