...
short   ConvertToBMP8(RImage* pImage)
{
   ...
   case RImage::SCREEN8_565:
   {
      ...
      *p565Pal = *pImage->m_pPalette;
      ...
      // The DIB when viewed as U32   Reserved|R|G|B
      //              viewed as 4 BYTES B|G|R|Reserved

      for (i = p565Pal->m_sStartIndex; i < p565Pal->m_sStartIndex + p565Pal->m_sNumEntries; i++)
         ulpDib[i] = (U32) (((usp565[i] & 0xf800) << 8) +    // Red
...
short   ConvertToBMP24(RImage* pImage)
{
   ...
   case RImage::SCREEN16_555:
   {
      ...
      pImage->m_sDepth = 24;
      pImage->m_lPitch = RImage::GetPitch(pImage->m_sWidth, pImage->m_sDepth);
      pImage->CreateData(pImage->m_lPitch * (S32)pImage->m_sHeight);
      UCHAR* ucp24 = (UCHAR*) pImage->m_pData;

      S32 sPitchWidth = sPitch / 2;
      for (r = 0; r < height; r++)
         for (c = 0; c < width; c++)
         {
...
short   ConvertToSCREEN8_555(RImage* pImage)
{
   ...
   case RImage::BMP8:
   {
      ...
      *pDibPal = *pImage->m_pPalette;
      ...
      U32* ulpDib = (U32*) pDibPal->m_pData;
      USHORT* usp555 = (USHORT*) pImage->m_pPalette->m_pData;

      short i;

      // The DIB format when viewed as a U32 is Reserved|R|G|B
...
short ConvertToSCREEN16_555(RImage* pImage)
{
   ...
   case RImage::BMP8:
   {
      ...
      U32* ulpPal = (U32*) pImage->m_pPalette->m_pData;
      ...
      USHORT* usp16 = (USHORT*) pImage->m_pData;
      UCHAR ucIndex;
      S32 dPitch = pImage->m_lPitch / (pImage->m_sDepth / 8);
...
short ConvertToSCREEN24_RGB(RImage* pImage)
{
   ...
   case RImage::BMP8:
   {
      S32 r, c, height, width, sPitch, dPitch;
      ...
      UCHAR* ucp8 = pImage->m_pData;
      U32* ulPal = (U32*) pImage->m_pPalette->m_pData;
...
short ConvertToSCREEN32_ARGB(RImage* pImage)
{
   ...
   case RImage::BMP8:
   {
      ...
      pImage->CreateData(pImage->m_lPitch * (S32)pImage->m_sHeight);
      U32* ulp32 = (U32*) pImage->m_pData;
      S32 dLongPitch = pImage->m_lPitch / 4;
   ...
   case RImage::BMP24:
   {
      ...
      for (r = 0 ; r < height; r++)
         for (c = 0; c < width; c++)
            ulp32[r * dLongPitch + c] = (U32) (
               ((U32) ucp24[r * sPitch + 3 * c + 2] * 0x10000) +
               ((U32) ucp24[r * sPitch + 3 * c + 1] * 0x100) +
               ((U32) ucp24[r * sPitch + 3 * c + 0]));
