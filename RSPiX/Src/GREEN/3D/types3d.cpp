   ...
   S32 linc)
{
   ...
   for (short i = 0; i < m_sNum; i++)
   {
      m_pIndices[i] = rspMatchColorRGB(
         S32(m_pColors[i].u8Red),
         S32(m_pColors[i].u8Green),
         S32(m_pColors[i].u8Blue),
...
RTexture::Adjust(
   ...
   S32 lInc)            // In:  Number of colors to skip.
{
   ASSERT(m_pColors);
...
void RMesh::Alloc(short sNum)
{
   Free();
   m_sNum = sNum;
   m_pArray = (USHORT*)calloc((S32)m_sNum * 3, sizeof(USHORT));
   ASSERT(m_pArray != 0);
}
...
short RMesh::Save(RFile* fp)
{
   short sResult = 0;
   fp->Write(&m_sNum);
   fp->Write(m_pArray, (S32)m_sNum * 3);
   if (fp->Error())
   {
      sResult = -1;
