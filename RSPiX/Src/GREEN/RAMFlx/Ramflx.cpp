   ...
   CImage* pimage)               // Memory allocated within struct if not NULL
{
   ...
   if (file.Open(pszFileName, "rb", ENDIAN_LITTLE) == 0)
   {
      ...
      if (ReadHeader(&file) == 0)
      {
         ...
         if (pimage == NULL)
         {
            sError = AllocBuf(&m_imagePrev, (S32)m_filehdr.sWidth, (S32)m_filehdr.sHeight, 256);
         }
      }
      else
         sError = 1;
   ...
   if ((sError == 0) && (pimage != NULL))
      sError = CreateBuf(pimage, (S32)m_filehdr.sWidth, (S32)m_filehdr.sHeight, 256);
...
//
///////////////////////////////////////////////////////////////////////////////
short CRamFlx::CreateBuf(CImage* pimage, S32 lWidth, S32 lHeight, short sColors)
{
   InitBuf(pimage);
