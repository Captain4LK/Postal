...
typedef void (*TaskFunc)(        // Returns nothing.
   U32 ulUser);                  // User defined m_ulUser.

// Typedef to override this RTask's timer.  If overridden, this RTask
// will use this callback to get the time.
typedef S32 (*TimeFunc)(         // Returns time as a S32 in milliseconds.
   S32 lTimeUser);               // User defined m_lTimeUser.
...
void Init(TaskFunc tf, U32 ulUser);
// Kill task info.
...
// Use a custom timer.
void SetTimeFunc(TimeFunc fnTime, S32 lTimeUser)
...
S32 m_lNextExpiration;                 // Next time to call task.
