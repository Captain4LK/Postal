...
short RLaymage::AllocateChannelBuffers(U32 ulSize)
{
...
short RLaymage::ReadLayerName(short sLayerNum, RFile* pcfLayer)
{
   ...
   pulChannelLength = (U32*) calloc(usNumChannels, sizeof(U32));
...
short RLaymage::RLE_Decompress(char* pcBuffer, U32 ulCompSize, RFile* pcfRLE)
{
   short sReturn;
   U32 ulRead = 0;
   U32 ulBufferPos = 0;
...
short RLaymage::ConvertToImage(short sLayerNum, U32 ulTop, U32 ulBottom,
                               U32 ulLeft, U32 ulRight)
{
   short sReturn = SUCCESS;
   RImage* pImage;
   U32 row;
   ...
   if (pImage)
   {
      ...
      pImage->m_sDepth = 32;
      pImage->m_lPitch = (S32)pImage->m_sWidth * (pImage->m_sDepth / 8);

      U32* ulp32 = (U32*) pImage->m_pData;
      S32 lDestPitch = pImage->m_lPitch / 4;
