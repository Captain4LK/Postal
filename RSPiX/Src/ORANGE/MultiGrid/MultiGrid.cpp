                      ...
                      short sW, short sH)
{
   ...
   for (j = 0; j < sH; j++)
      for (i = 0; i< sW; i++)
      {
         short sValue = m_psGrid[i + sSrcX + (j + sSrcY) * S32(m_sWidth)];
...
short RMultiGrid::Alloc(short sW, short sH)
{
   ...
   if (!(m_psGrid = (short*) calloc(sizeof(short), S32(sW) * sH) )) return FAILURE;
...
short RMultiGrid::AllocGrid(short sScaleW, short sScaleH)
{
   ...
   if (!(m_psGrid = (short*) calloc(sizeof(short), S32(sGridW) * sGridH) )) return FAILURE;
...
short RMultiGrid::Compress(
   ...
   short sTileH,
   S32* plSize,         // New Data Size (BYTES)
   S32* plNumBlocks,    // Number of unique tiles needed
   ...
   )
{
   ...
   for (sFullY = 0, sBlockY = 0; sBlockY < sGridH; sBlockY++, sFullY += m_sMaskY + 1)
   {
      ...
      for (sBlockX = 0; sBlockX < sGridW; sBlockX++)
      {
         ...
         short* psSrcBlock = psUncompressedData + S32(sBlockX) * (m_sMaskX + 1) +
                             S32(sFullY) * m_sWidth;
         ...
         for (j = 0; j < sScanH; j++)
         {
            for (i = 0; i < sScanW; i++) // only copy what's needed...
            {
               ...
               sVal = psSrcBlock[i + j * S32(m_sWidth)];
   ...
   S32 lLen = sShortSize * sizeof(short) * sTileNumber;
   ...
   if (plSize)
      *plSize = S32(sTileNumber) * (S32(sShortSize) * sizeof(short) + sizeof(short*))
                + S32(sGridH) * (sGridW * sizeof(short) + sizeof(short*) );
...
short RMultiGrid::Decompress()
{
   ...
   short *psNewGrid = (short*) calloc(sizeof(short), S32(m_sWidth) * m_sHeight);
   ...
   for (j = 0; j < m_sHeight; j++)
   {
      for (i = 0; i < m_sWidth; i++)
      {
         psNewGrid[i + S32(j) * m_sWidth ] = GetVal(i, j);
      }
   }
...
short RMultiGrid::Save(RFile* fp)
{
   ...
   fp->Write(S32(0)); // Reserved1
   fp->Write(S32(0)); // Reserved2
   fp->Write(S32(0)); // Reserved3
   fp->Write(S32(0)); // Reserved4
...
short RMultiGrid::Load(RFile* fp)
{
   ...
   // ALLOCATE IT
   m_psGrid = (short*) calloc(sCodeSize, S32(sGridW) * sGridH);
   if (!m_psGrid)
   {
