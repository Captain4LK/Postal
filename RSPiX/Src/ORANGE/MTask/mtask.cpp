...
static S32 m_lMainProgramStack;     // save Main program stack here
static S32 m_lTaskStack;            // save Current task stack here
...
short MTaskAddFunc(void* pFunction, char* pszFuncName, short sStackSize)
{
   ...
   if (ptiNewTask != NULL)
   {
      plNewStack = (S32*) calloc(sLongElements, 4);
      if (plNewStack)
      {
         plNewStack[sLongElements - 1] = (S32) MTaskReturnCatcher;
         plNewStack[sLongElements - 2] = (S32) pFunction;
