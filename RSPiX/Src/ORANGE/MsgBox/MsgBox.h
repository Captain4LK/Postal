...
typedef U32 (*MsgBoxCall)(       // If the return value is non-zero,
...
U32 DoModal(                              // Returns chosen ID on success,
                                          // 0 on failure.
...
U32 DoModeless(               // Returns item clicked or 0, if none.
   RInputEvent* pie);         // In:  Most recent user input event.
                              // Out: pie->sUsed = TRUE, if used.
...
U32 m_ulId;                   // ID of control that caused DoModal to end.
