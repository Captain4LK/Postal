...
{
   S32 lEntireFileSize;       // Size of entire file, including header
   ...
   S32 lMilliPerFrame;           // FLC: milliseconds between frames (4 UCHARs)
   ...
   U32 dCreatedTime;          // MS-DOS-formatted date and time of file's creation
   U32 dCreator;              // Serial number of Animator Pro program used to
                              // create file -- $464c4942 is a good one ("FLIB")
   U32 dUpdatedTime;          // MS-DOS-formatted date and time of file's update
   U32 dUpdater;              // Serial number of Animator Pro program used to
   ...
   S32 lOffsetFrame1;            // Offset from beginning of file to first frame chunk
   S32 lOffsetFrame2;            // Offset from beginning of file to second frame chunk,
...
{
   S32 lChunkSize;               // Size of entire frame chunk, including header
...
{
   S32 lChunkSize;               // Size of frame data chunk, including header
...
short WriteColorDelta(FLX_BUF* pbufNext, FLX_BUF* pbufPrev, UCHAR* pBuf, S32* plChunkSize);
short WritePixelDelta(FLX_BUF* pbufNext, FLX_BUF* pbufPrev, UCHAR* pBuf, S32* plChunkSize);
short WriteDataChunk(UCHAR* pData, S32 lSize, USHORT wType, S32* plChunkSize);
...
short CompressLineDelta(
   ...
   S32& lSize,             // Size of the data used by current compressed line.
