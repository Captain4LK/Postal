...
extern short rspGetMouseEvent(   // Returns 0 if no event was available, non-zero otherwise
   ...
   S32* plTime = NULL,              // Event's time stamp returned here (unless NULL)
...
extern short rspGetKey(                // Returns 1 if key was available, 0 if not
   S32* plKey,                      // Out: Key info (0 if no key was available)
   S32* plTime = NULL);             // Out: Key's time stamp (unless NULL)
...
extern void rspGetJoyPos(
   short sJoy,             // In:  Joystick to query.
   S32 *plX,               // Out: X axis position of joystick, if not NULL.
   S32 *plY = NULL,        // Out: Y axis position of joystick, if not NULL.
   S32 *plZ = NULL,        // Out: Z axis position of joystick, if not NULL.
   S32 *plW = NULL,        // Out: W axis position of joystick, if not NULL.
   S32 *plU = NULL,        // Out: U axis position of joystick, if not NULL.
   S32 *plV = NULL);    // Out: V axis position of joystick, if not NULL.
...
extern void rspSetPaletteEntries(
   ...
   S32 lIncBytes);                     // In:  What to add to pointers to move to next value
...
extern S32 rspGetMicroseconds(         // Returns time in microseconds
   short sReset = FALSE);              // In:  TRUE to reset count, FALSE otherwise

extern S32 rspGetMilliseconds(void);   // Returns time in milliseconds
...
extern short rspSetSoundOutMode(          // Returns 0 if successfull, non-zero otherwise
   S32 lSampleRate,                       // In:  Sample rate
   S32 lBitsPerSample,                    // In:  Bits per sample
   S32 lChannels,                      // In:  Channels (mono = 1, stereo = 2)
   S32 lCurBufferTime,                    // In:  Current buffer time (in ms.)
   S32 lMaxBufferTime,                    // In:  Maximum buffer time (in ms.)
   RSP_SND_CALLBACK callback,             // In:  Callback function
   U32 ulUser);                           // In:  User-defined value to pass to callback

extern short rspGetSoundOutMode(          // Returns 0 if successfull, non-zero otherwise
   S32* plSampleRate,                     // Out: Sample rate or -1 (unless NULL)
   S32* plBitsPerSample = NULL,           // Out: Bits per sample or -1 (unless NULL)
   S32* plChannels = NULL,             // Out: Channels (mono=1, stereo=2) or -1 (unless NULL)
   S32* plCurBufferTime = NULL,           // Out: Current buffer time or -1 (unless NULL)
   S32* plMaxBufferTime = NULL);       // Out: Maximum buffer time or -1 (unless NULL)

extern void rspSetSoundOutBufferTime(
   S32 lCurBufferTime);                // In:  New buffer time
...
extern S32 rspGetSoundOutPos(void);    // Returns sound output position in bytes

extern S32 rspGetSoundOutTime(void);      // Returns sound output position in time

extern S32 rspDoSound(void);
