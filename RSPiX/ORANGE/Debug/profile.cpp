...
#if WIN32
S64 i64GetTimeSpeed = 7;   // how S32 does the time command take?
#else
...
void RProfile::Report()
{
   ...
   if (!m_lDeceasedTime) // degenerate report at end:
   {
      ...
      if (m_lCount) // safety
         fprintf(fp, "Profiler overhead: Tot(ms) = %g, # of calls = %" PRId32 ", Avg(ms) = %g\n\n",
                 double(m_lTotTime) / 1000.0, S32(m_lCount), double(m_lTotTime) / double(m_lCount * S64(1000)));
      ...
      for (i = 0; i < m_sNumTracked; i++)
      {
         if (m_aList[i].m_lNumCalls) // safety
         {
            fprintf(fp, "-------------------------------------------------------\n%s:\n",
                    m_aList[i].m_szFieldName);
            fprintf(fp, "            # of passes = %" PRId32 ", Tot(ms) = %g, Avg(ms) = %g",
            ...
            if (dRel)
            {
               ...
               fprintf(fp, "  [%+1.6g %%]\n", double( S32(dRat * 10000.0) ) / 10000.0);
            }
            else
               fprintf(fp, "\n");
