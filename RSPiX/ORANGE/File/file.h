...
typedef void (*CritiCall)(       // Returns nothing.
   S32 lBytes);                  // Bytes about to be read or written.
...
// Returns 0 on success.
short Open(                   // Returns 0 on success.
   S32 lSize,                 // Size in bytes to begin with.
   S32 lGrowSize,             // Min amount in bytes to grow memory file when written passed end.
...
// functionality.
static void SetOpenHook(OpenHook hook, S32 lUser)
...
// functionality.
static void SetCloseHook(CloseHook hook, S32 lUser)
...
// Returns 0 on success.
short Seek(S32 lPos, S32 lOrigin);
...
S32 Write(const void* pData, S32 lNum);
...
// including the NULL terminator (unlike strlen()).
S32 Write(const char* pszString);
...
// pointer overloaded by other Write(type*, S32) RFile functions.
////////////////////////////////////////////////////////////////////////
...
// Writes one U16.
S32 Write(U16 u16Val)
{ return Write(&u16Val); }

// Writes one S16.
S32 Write(S16 s16Val)
{ return Write(&s16Val); }

// Writes one RPixel24.
S32 Write(RPixel24 pix24Val)
{ return Write(&pix24Val); }

// Writes one U32.
S32 Write(U32 u32Val)
{ return Write(&u32Val); }

// Writes one S32.
S32 Write(S32 s32Val)
{ return Write(&s32Val); }

// Writes one RPixel32.
S32 Write(RPixel32 pix32Val)
{ return Write(&pix32Val); }

// Writes one float.
S32 Write(float fVal)
{ return Write(&fVal); }

// Writes one U64.
S32 Write(U64 u64Val)
{ return Write(&u64Val); }

// Writes one S64.
S32 Write(S64 s64Val)
{ return Write(&s64Val); }

// Writes one double.
S32 Write(double dVal)
{ return Write(&dVal); }
...
S32 Read24(             // Returns number of 24 bit items read.
   RPixel24* ppix24,       // In:  24 bit data to read (swapping, if necessary).
   S32 lNum);              // In:  Number of 24 bit items to read.
...
S32 Write24(               // Returns number of 24 bit items written.
