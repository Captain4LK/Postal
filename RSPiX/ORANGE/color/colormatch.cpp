...
{
   S32 lMatch = 0, i;
   S32 lMax =  S32(16777217), lDist;
   S32 lOffset = linc * sStart; // array offset
   ...
   for (i = 0; i<sNum; i++)
   {
      lDist = SQR(S32(*pr) - r) + SQR(S32(*pg) - g) + SQR(S32(*pb) - b);
      pr += linc;
      pg += linc;
      pb += linc;
      if (lDist < lMax)
...
// Input description should be arrays at least sAlphaDepth S32.
short RAlpha::CreateLightEffectRGB(UCHAR* pa, UCHAR* pr, UCHAR* pg, UCHAR* pb, S32 linc,
   ...
   S32*   plAlignedSize)
{
   ...
   UCHAR*** pppucFastMem = (UCHAR***) aligned_alloc(4096, lTotSize + 4095);
   memset(pppucFastMem, 0, lTotSize + 4095);
   // WARNING:  these arrays of pointers are 4 bytes in size, so be careful
   // how you seek!

   // I am adding on in BYTES, not words...
   //UCHAR*** pppucFastAligned = (UCHAR***) ((U32(pppucFastMem)+4095) & (~4095) );
   UCHAR*** pppucFastAligned = pppucFastMem;

   if (!pppucFastMem) return NULL;
   UCHAR* pInfo = (UCHAR*)pppucFastAligned;
   // For freeing:
   S32 lMemOffset = uintptr_t(pppucFastAligned) - uintptr_t(pppucFastMem);
   ...
   // ( + 4LS bytes)
   UCHAR* pucData = (UCHAR*) (ppucFirstSrcArray + S32(m_sNumLevels) * sNumSrc);
   UCHAR* pData = pucData;
                                       ...
                                       S32* plDataSize)
{
   ...
   S32 lHeaderMem = 1024; // ucClear & deltaMemFree16, + 255 pointer array (UCHAR**)
   lDataPerLev = S32(sNumSrcCol) * sNumDstCol; // Src * Dst BYTE table
...
short RFastMultiAlphaWrapper::Save(RFile* pf)
{
   ...
   S32 lDataLen = S32(m_sNumLayers) * S32(m_sNumSrc) * S32(m_sNumDst);
...
short RFastMultiAlphaWrapper::Load(RFile* pf)
{
   ...
   // Assumes a fully correct this pointer!
   S32 lTotSize = 1024 + S32(m_sNumLayers) * m_sNumSrc * (4L + m_sNumDst);
   // align it to 4096 to save cache memory!
   //UCHAR*** pppucFastMem = (UCHAR***) calloc(1,lTotSize + 4095);
   UCHAR*** pppucFastMem = (UCHAR***) aligned_alloc(4096, lTotSize + 4095);
   memset(pppucFastMem, 0, lTotSize + 4095);
   ...
   // ( + 4LS bytes)
   UCHAR* pucData = (UCHAR*) (ppucFirstSrcArray + S32(m_sNumLayers) * m_sNumSrc);
   UCHAR* pData = pucData;
   ...
   /*
   S32 lNumSrcTable = m_sNumLayers * m_sNumSrc;
   S32 lSrcPtrOffset = m_sNumDst;

   UCHAR* pSrcTableValue = pData - m_sStartDst; // offset base dest

   for (S32 i=0;i < lNumSrcTable;i++,pSrcTableValue += lSrcPtrOffset)
      {
      *ppucCurSrc++ = pSrcTableValue;
      }

   //============  Populate the alpha table  ==================
   S32 lAlphaPtrOffset = m_sNumSrc; // SHOULD advance by 4
