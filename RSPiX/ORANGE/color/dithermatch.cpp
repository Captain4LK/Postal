...
short   rspDither(
   ...
   S32 lMilli              // milliseconds per callback
   )
{
   ASSERT(pimSrc);
   ASSERT(pimDst);
   ...
   S32 lSrcXP = 3; // hard coded for 24-bit bmp's!!!
   ...
   )
{
   ...
   for (j = sH; j; j--)
   {
      for (i = 0; i < sW; i++, pSrc += lSrcXP, pDst += sDirection, sErrPos += sDirection)
      {
         ...
         lRed += S32(sNextErrorR + psCurrentErrorR[sErrPos]);
         lGreen += S32(sNextErrorG + psCurrentErrorG[sErrPos]);
         lBlue += S32(sNextErrorB + psCurrentErrorB[sErrPos]);
   ...
   )
{
   ...
   S32 lSrcXP = 4; // hard coded for 32-bit bmp's!!!
