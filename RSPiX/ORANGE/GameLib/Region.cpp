...
RRectangularRegion::RRectangularRegion(S32 lX, S32 lY,
                                       S32 lLeft, S32 lRight, S32 lTop, S32 lBottom)
{
   m_lAbsX = lX;
   m_lAbsY = lY;
   m_lAbsZ = 0;
...
short RRectangularRegion::Collide(S32 lX, S32 lY)
{
   RPt point(lX, lY);
...
RCubicRegion::RCubicRegion(S32 lX, S32 lY, S32 lZ, S32 lLeft, S32 lRight,
                           S32 lTop, S32 lBottom, S32 lFront, S32 lBack)
{
   m_lAbsX = lX;
   m_lAbsY = lY;
   m_lAbsZ = lZ;
...
RCircularRegion::RCircularRegion(S32 lX, S32 lY,
                                 S32 lXCirc, S32 lYCirc, S32 lRadius)
{
   m_lAbsX = lX;
   m_lAbsY = lY;
   circle.X = lXCirc;
...
{
   S32 dx = pPoint->X - circle.X;
   S32 dy = pPoint->Y - circle.Y;
   S32 d2 = dx * dx + dy * dy;
...
{
   S32 dx = circle.X - pRegion->circle.X;
   S32 dy = circle.Y - pRegion->circle.Y;
   S32 d2 = dx * dx + dy * dy;
   S32 r2 = circle.lRadius + pRegion->circle.lRadius;
   r2 *= r2;
   if (d2 > r2)
      return NO_COLLISION;
   else
...
RSphericalRegion::RSphericalRegion(S32 lX, S32 lY, S32 lZ,
                                   S32 lXSphere, S32 lYSphere, S32 lZSphere,
                                   S32 lRadius)
{
   m_lAbsX = lX;
   m_lAbsY = lY;
...
{
   S32 dx = sphere.X + m_lAbsX - pPoint->X;
   S32 dy = sphere.Y + m_lAbsY - pPoint->Y;
   S32 dz = sphere.Z + m_lAbsZ - pPoint->Z;
   S32 r2 = sphere.lRadius * sphere.lRadius;
...
short RSphericalRegion::Collide(R3DLine* pline)
{
         ...
         // it says 'S32 lCirDist = SQR(lRelCenX) + SQR(lRelCenX);'
   ...
   S32 lDelX = (pline->X1 - pline->X2);
   S32 lDelY = (pline->Y1 - pline->Y2);
   // JMI: Added Z component:
   S32 lDelZ = (pline->Z1 - pline->Z2);
   ...
   if ( (lDelX * lRelCenX + lDelY * lRelCenY + lDelZ * lRelCenZ) >= 0)
   {
      ...
      // 2) is it in my range?:
      // JMI: Added Z component:
      S32 lCirDist = SQR(lRelCenX) + SQR(lRelCenY) + SQR(lRelCenZ);
      // JMI: Added Z component:
      S32 lLineLen = SQR(lDelX) + SQR(lDelY) + SQR(lDelZ);
      ...
      if (lLineLen > (lCirDist + SQR(sCirR) - 2 * sCirR * dCirLen) )
      {
         S32 a = lLineLen;
         // JMI: Added Z component:
         S32 b = -2 * (lRelCenX * lDelX + lRelCenY * lDelY + lDelZ * lRelCenZ);
         S32 c = lCirDist - SQR(sCirR);
...
{
   S32 dx = sphere.X + m_lAbsX - pRegion->sphere.X + pRegion->m_lAbsX;
   S32 dy = sphere.Y + m_lAbsY - pRegion->sphere.Y + pRegion->m_lAbsY;
   S32 dz = sphere.Z + m_lAbsZ - pRegion->sphere.Z + pRegion->m_lAbsZ;
   S32 d2 = dx * dx + dy * dy + dz * dz;
   S32 r2 = sphere.lRadius + pRegion->sphere.lRadius;
   r2 *= r2;
   if (d2 > r2)
      return NO_COLLISION;
   else
