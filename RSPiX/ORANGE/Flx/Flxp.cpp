   ...
   if (m_file.Error() == FALSE && framehdr.wType == 0xF1FA)
   {
      ...
      for (short sSub = 0; sSub < framehdr.sNumSubChunks && sError == 0; sSub++)
      {
         ...
         if (m_file.Error() == FALSE)
         {
            ...
            S32 lDataSize = datahdr.lChunkSize - 6;
...
short CFlx::ReadDataLC(FLX_BUF* pbufRead)
{
   ...
   pbRow = pbufRead->pbPixels + ((U32)y * (U32)pbufRead->sPitch);
...
// For debugging, prefetch a bunch of values to view them in the debugger
         ...
         pbPix = pbufRead->pbPixels + ((U32)y * (U32)pbufRead->sPitch);
...
//               memset(pbPix, (int)wVal, (size_t)sCount);
         ...
         if (bLastByte == TRUE)
         {
            ...
            pbPix = pbufRead->pbPixels + (((U32)y + 1L) * (U32)pbufRead->sPitch) - 1L;
...
//
///////////////////////////////////////////////////////////////////////////////
short CFlx::WriteColorDelta(FLX_BUF* pbufNext, FLX_BUF* pbufPrev, UCHAR* pBuf, S32* plChunkSize)
{
   short sError = 0;
...
short CFlx::WritePixelDelta(FLX_BUF* pbufNext, FLX_BUF* pbufPrev, UCHAR* pBuf, S32* plChunkSize)
{
   ...
   *plChunkSize = 0;
   ...
   for (short y = 0; y < m_filehdr.sHeight; y++)
   {
      ...
      pbSrc += (U32)pbufNext->sPitch;     // Use pitch on source
      pbDst += (U32)m_filehdr.sWidth;     // Use width on buffer
   }
   S32 lSize = (S32)m_filehdr.sWidth * (S32)m_filehdr.sHeight;
...
#if 0
   S32 lSize = CompressBRUN(pbufNext->pbPixels, pbufNext->sPitch,
   ...
   if (pbufPrev == NULL)
   {
      ...
      S32 lSizeBRUN = CompressBRUN(pbufNext->pbPixels, pbufNext->sPitch,
                                   0, 0, m_filehdr.sWidth, m_filehdr.sHeight, pBuf);
      S32 lSizeCOPY = (S32)m_filehdr.sWidth * (S32)m_filehdr.sHeight;
   ...
   else
   {
      ...
      if (m_filehdr.wMagic == FLX_MAGIC_FLI)
      {
         ...
         S32 lSize;                       // size of the current compressed line
         ...
         S32 lSizeLC = 4;                 // size of the chunk in pBuf
      ...
      else
      {
         ...
         S32 lSizeSS2 = 2;                // size of the chunk in pBuf
...
{
   // Local variables.
   U32 dwOffset;                    // Offset into the pixel data.
   ...
   dwOffset = (U32)y * (U32)pbufNext->sPitch;
   ...
   else if (sAlign == 2)
   {
      ...
      if (sLineSkipCount > 0)
      {
         ...
         *(short*)(pbDst + (U32)lSize) = -sLineSkipCount;
         lSize += 2;
      }
...
//      {
      ...
      if (pbSrcNext[dwOffset] != pbSrcPrev[dwOffset])
      {
         ...
         *(USHORT*)(pbDst + (U32)lSize) = wLastByte;
         lSize += 2;
      }
   ...
   while (((sAlign == 1) && (x < sAdjustedPitch)) || ((sAlign == 2) && (x < sAdjustedPitch - 1)))
   {
      ...
      if (((sAlign == 1) && (x < sAdjustedPitch)) || ((sAlign == 2) && (x < sAdjustedPitch - 1)))
      {
         ...
         if (sAlign == 1)
         {
            ...
            dwOffset = (U32)y * (U32)pbufNext->sPitch + x - sSkipCount;
            ...
            while (sSkipCount > 255)
            {
               ...
               *(pbDst + (U32)lSize++) = 1;
               ...
               dwOffset += 255;
               *(pbDst + (U32)lSize++) = pbSrcNext[dwOffset];
            ...
            *(pbDst + (U32)lSize++) = (UCHAR)sSkipCount;
            sSkipCount = 0;
            ...
            if ((x < (sAdjustedPitch - 1)) && (pbSrcNext[dwOffset + x] == pbSrcNext[dwOffset + x + 1]))
            {
               ...
               nBytes = 255 - nBytes + 1;
               *(pbDst + (U32)lSize++) = nBytes;
               *(pbDst + (U32)lSize++) = pbSrcNext[dwOffset + (x - 1)];
               sPacket++;
            }
            else
            {
               // Let's process as count of different bytes.
               pbByteCount = pbDst + (U32)lSize++;
               *(pbDst + (U32)lSize++) = pbSrcNext[dwOffset + x];
               x++;
         ...
         else
         {
            ...
            *(pbDst + (U32)lSize++) = (UCHAR)sSkipCount;
                ...
                (pbSrcNext[dwOffset + x + 1] == pbSrcNext[dwOffset + x + 3]))
            {
               ...
               nBytes = 255 - nBytes + 1;
               *(pbDst + (U32)lSize++) = nBytes;
               *(pbDst + (U32)lSize++) = pbSrcNext[dwOffset + x - 2];
               *(pbDst + (U32)lSize++) = pbSrcNext[dwOffset + x - 1];
               sPacket++;
            }
            else
   ...
   UCHAR* pbOut)        // Pointer to output (compressed data)
{
   ...
   pbIn = pbIn + ((U32)sSrcY * (U32)sPitch) + (U32)sSrcX;
...
//
///////////////////////////////////////////////////////////////////////////////
short CFlx::WriteDataChunk(UCHAR* pbData, S32 lSize, USHORT wType, S32* plChunkSize)
{
   FLX_DATA_HDR datahdr;
...
short CFlx::ReadHeader(void)
{
   ...
   else
   {
      ...
      m_filehdr.lMilliPerFrame = (S32)( (double)sJiffies * ((double)1000 / (double)70L) + (double)0.5 );
...
void CFlx::CopyBuf(FLX_BUF* pbufDst, FLX_BUF* pbufSrc)
{
   ...
   for (short y = 0; y < m_filehdr.sHeight; y++)
   {
      ...
      pbSrc += (U32)pbufSrc->sPitch;
      pbDst += (U32)pbufDst->sPitch;
   }
