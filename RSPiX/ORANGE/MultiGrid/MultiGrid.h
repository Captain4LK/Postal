...
void   SetValueUncompressed(short sVal, short sX, short sY)
{
   ...
   *(m_psGrid + sX + S32(sY) * m_sWidth) = sVal;
}
...
short GetValueUncompressed(short sX, short sY)
{
   ...
   return *(m_psGrid + sX + S32(sY) * m_sWidth);
}
...
short Compress(
   ...
   short sTileH,
   S32* plSize = NULL,        // New Data Size (BYTES)
   S32* lNumBlocks = NULL,   // Number of unique tiles needed
