                                ...
                                short sTileW, short sTileH)
{
   ...
   m_pucPalette = (UCHAR*) calloc(sMaxPlanes, S32(sGridW) * sGridH);
   ASSERT(m_pucPalette);
   m_plAccessX = (U32*) calloc(sizeof(S32), sGridW * sTileW);
   ASSERT(m_plAccessX);
   m_ppucAccessY = (UCHAR**) calloc( sizeof(UCHAR*), S32(sGridH) * sTileH );
   ASSERT(m_ppucAccessY);
   ...
   m_lTileLen = S32(m_sTileW) * m_sTileH;
...
short RMultiGridIndirect::Load(RFile* fp)
{
   ...
   S32 lLen = S32(m_sGridW) * m_sGridH * m_sMaxPlanes;
   ...
   m_plAccessX = (U32*) calloc(sizeof(S32), m_sGridW * m_sTileW);
   ASSERT(m_plAccessX);
   m_ppucAccessY = (UCHAR**) calloc( sizeof(UCHAR*), S32(m_sGridH) * m_sTileH );
   ASSERT(m_ppucAccessY);
