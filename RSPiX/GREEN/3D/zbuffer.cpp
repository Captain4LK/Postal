...
short RZBuffer::Create(short sW, short sH)
{
   ...
   m_sW = sW;
   m_sH = sH;
   m_lP = S32((sW + 3) & ~3); // keep for emergencies!

   S32 lSize = m_lP * m_sH * sizeof(short);
...
void RZBuffer::Clear(short sVal)
{
   ...
   //2) Do the copy
   S32 lWordP = m_lP >> 2; // 4 * 16 = 64
   U64* pWord = (U64*) m_pBuf;

   S32 lWordLen = lWordP * m_sH;
   for (S32 i = 0; i < lWordLen; i++) *pWord++ = BigWord.word;
