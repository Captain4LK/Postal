...
short OpenChannel(S32 lSampleRate,
                  S32 lBitsPerSample,
                  S32 lNumChannels);
...
static short SetMode(               // Returns 0 on success.
   ...
   S32 lDevBitsPerSample,           // Number of bits per sample for device.
...
static short SetMode(               // Returns 0 on success.
   S32 lSamplesPerSec,              // Sample rate in samples per second.
   S32 lBitsPerSample,              // Number of bits per sample.
   S32 lNumChannels,                // Number of channels (1 == mono,2 == stereo).
   S32 lBufferTime,                 // Amount of time buffer spends in queue b4
                                    // being played.
   S32 lMaxBufferTime)              // Maximum that lBufferTime can be set to
...
static S32 Do(void);       // Returns value returned by rspDoSound() that
...
// to rspDoSound.
static void SetBufferTime(S32 lBufferTime)
...
static S32 ms_lCurPos;                       // Current play position
                                             // based on absolute start.
static U32 ms_ulBufSize;                  // The size to use when allocating
                                          // RMixBufs.
