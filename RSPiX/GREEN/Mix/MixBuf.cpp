...
//////////////////////////////////////////////////////////////////////////////
S32 RMixBuf::ms_lSampleRate;           // Sample rate for audio
                                       // playback/mix.
S32 RMixBuf::ms_lSrcBitsPerSample;     // Sample size in bits for sample data.
                                       // 0 for no preference.
S32 RMixBuf::ms_lMixBitsPerSample;     // Sample size in bits for mixing.
S32 RMixBuf::ms_lDstBitsPerSample;     // Sample size in bits for Blue data.
S32 RMixBuf::ms_lNumChannels;          // Number of channels (mono
                                       //  or stereo).
...
inline void Conv8to16(  // Returns nothing.
   ...
   S32 lSamples)        // In:  Samples to convert.
{
   while (lSamples--)
   {
...
inline void Mix(     // Returns nothing.
   U8*   pu8Src,     // In:  Src.
   U8*   pu8Dst,     // In:  Dst.
   S32 lSamples)     // In:  Number of samples to mix.
{
   short sVal;
...
RMixBuf::RMixBuf(
   ...
   U32 ulSize) // In:  Size of destination buffer in bytes.
{
   ms_sNumBufs++;
...
//
//////////////////////////////////////////////////////////////////////////////
short RMixBuf::SetSize(U32 ulSize)
{
   ...
   U32 ulSize)       // In:  Size of destination buffer in bytes.
{
   ...
   if (m_ulMixSize != m_ulDstSize && m_ulDstSize)
   {
      ...
      else
      {
         ...
         S32 lNumSamples = m_ulDstSize * 8 / ms_lDstBitsPerSample;
         S32 lSize = lNumSamples * ms_lMixBitsPerSample / 8;
         if (SetSize(lSize) == 0)
         {
...
//
//////////////////////////////////////////////////////////////////////////////
short RMixBuf::Mix(   U32 ulStartPos,
                      U8*      pu8Data,
                      U32 ulSize,
                      S32 lSampleRate,
                      S32 lBitsPerSample,
                      ...
                      UCHAR ucVol2)
{
   ...
   if (m_ulMixSize != 0)
   {
      ...
      U32 ulNum   = MIN(ulSize, m_ulMixSize - ulStartPos);
