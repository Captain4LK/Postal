...
short   ConvertToFSPR8(RImage*  pImage)
{
   ...
   S32 lSizeEstimate = ((S32)(pImage->m_sHeight + 1)) * (pImage->m_sWidth * 2 + 1) + 15;
   //pHeader->m_pCompMem = (UCHAR*)malloc((size_t)pImage->m_sHeight*(size_t)pImage->m_sWidth+15);
   pHeader->m_pCompMem = (UCHAR*)aligned_alloc(16, (size_t)pImage->m_sHeight * (size_t)pImage->m_sWidth + 15);

   pHeader->m_pCompBuf = pHeader->m_pCompMem;
   //pHeader->m_pCompBuf = (UCHAR*)(( (S32)(pHeader->m_pCompMem) + 15) & ~ 15); // align it 128!
   ...
   for (y = 0; y<sH; y++)
   {
      ...
      for (x = 0, pucBPos = pImage->m_pData + (S32)sP * (S32)y; x<sW; x++)
      {
   ...
   // NOTE THE SIZE:
   pHeader->m_lBufSize = U32(pucCPos - pHeader->m_pCompBuf);
   ...
   for (y = 0; y<sH; y++)
   {
      ...
      pucBPos = pImage->m_pData + (S32)sP * (S32)y;
      x = 0;
...
NextRun:
   ...
   // NOTE THE SIZE:
   pHeader->m_lCodeSize = U32(pucConBlk - pHeader->m_pCodeBuf);
   ...
   //Captain4LK: TODO these casts could break the code, depending on how much is allocated
