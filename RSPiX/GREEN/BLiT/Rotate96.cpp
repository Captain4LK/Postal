...
inline void _BlitRot(short sDeg, short sHeight, // = 2R + 1?
                     UCHAR* pSrcData, S32 lSrcxp, S32 lSrcp,
                     UCHAR* pDstData, S32 lDstXP, S32 lDstP,
                     ...
                     short sClipL, short sClipR, short sClipT, short sClipB)
{
   ...
   S32 lDen = S32(sDstW) * sDstH;   // compound fraction

   // Calculate the sub pixel offset:
   S32 lLadNumX, lLadNumY, lRungNumX, lRungNumY;
   ...
   // 32-bit version
   S32 lPX = lDen * sR, lPY = lDen * sR; // go into Denominator space
   // Stay in denominator space!
   lPX += S32(COSQ[sDegP] * sR * lDen + 0.5 * lDen); // offset to pixel center
   lPY += S32(SINQ[sDegP] * sR * lDen  + 0.5 * lDen); // offset to pixel center
   // Now MUST convert to an asymettrical signed proper fraction:
   ...
   S32 lRungW = S32(COSQ[sDegR] * sEdge * sDstH); // horizontal
   S32 lRungH = S32(SINQ[sDegR] * sEdge * sDstH);
   S32 lLadW = S32(COSQ[sDegL] * sEdge * sDstW);  // vertical
   S32 lLadH = S32(SINQ[sDegL] * sEdge * sDstW);
   ...
   //
   S32 lRungIncX = lRungW;
   S32 lRungIncY = lRungH;
   S32 lLadIncX = lLadW;
   S32 lLadIncY = lLadH;
   // General parameters:
   S32 lRungDelX = 0, lRungDelY = 0, lLadDelX = 0, lLadDelY = 0;
...
#define sProperRungY 0
   ...
   //==================================================================
   // relative to pP:
   S32 lLadX = 0, lLadY = 0, lRungX, lRungY;
...
short rspAddRotationPadding(RImage* pimSrc, short sHotX, short sHotY)
{
   ...
   S32 lR = 0; // squared at first...

   S32 lHotXS = SQR((S32)sHotX);
   S32 lHotYS = SQR((S32)sHotY);
   ...
   //
   lR = MAX(lR, (S32)SQR((S32)sHotX - (pimSrc->m_sWidth - 1) ) +
            (S32)SQR((S32)sHotY - (pimSrc->m_sHeight - 1) ) ); // LR corner
   lR = MAX(lR, lHotXS + (S32)SQR((S32)sHotY - (pimSrc->m_sHeight - 1) ) ); // UR corner
   lR = MAX(lR, SQR(sHotX - (pimSrc->m_sWidth - 1) ) + lHotYS ); // LL corner
   lR = MAX(lR, lHotXS + lHotYS ); // UL corner

   lR = S32(0.999999 + sqrt(double(lR) * 2.0)); // round up
...
short rspRemovePadding(RImage* pimSrc)
{
   ...
   RImage imDst;
   S32 lNewPitch = (pimSrc->m_sWinWidth + 15) & ~15; // 128 bit align it
                 ...
                 const RRect* prDstClip)
{
   ...
   //
   S32 lDstP = pimDst->m_lPitch, lDstXP = (pimDst->m_sDepth >> 3);
