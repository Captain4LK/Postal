...
short   ConvertToFSPR8(RImage*  pImage)
{
   ...
   S32 lSizeEstimate = ((S32)(pImage->m_sHeight + 1)) * pImage->m_sWidth * 2 + 15;
   pHeader->pCMem = (UCHAR*)malloc((size_t)pImage->m_sHeight * (size_t)pImage->m_sWidth + 15);

   pHeader->pCBuf = (UCHAR*)(( (S32)(pHeader->pCMem) + 15) & ~15);  // align it 128!
   ...
   // NOTE THE SIZE:
   pHeader->m_lBufSize = U32(pucCPos - pHeader->pCBuf);
   ...
   // And align it:
   pHeader->pCBuf = (UCHAR*)(( (S32)(pHeader->pCMem) + 15) & ~15);
   ...
   for (y = 0; y<sH; y++) pHeader->pLineArry[y] += (S32)(pHeader->pCBuf);
...
NextRun:
   ...
   // NOTE THE SIZE:
   pHeader->m_lCodeSize = U32(pucConBlk - pHeader->pControlBlock);
   ...
   for (y = 0; y<sH; y++) pHeader->pCtlArry[y] += (S32)(pHeader->pControlBlock);
