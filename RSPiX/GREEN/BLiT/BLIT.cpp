...
void   rspClipPlot(PIXSIZE color, RImage* pimDst, short sX, short sY, const RRect* prClip)
{
   ...
   //Captain4LK: TODO are all these intptr_t casts doing what they are supposed to?
   switch ((short)(((intptr_t)pimDst->m_pSpecial))) // 0 = normal image
   {
...
template void rspClipPlot<U32>(U32 color, RImage* pimDst, short sX, short sY, const RRect* prClip);
...
void instantiatePlot()
{
   ...
   rspPlot((U32)0, pim, (short)0, (short)0);
...
template short rspLasso<USHORT>(USHORT ignoreColor, RImage* pimSrc, short &x, short &y, short &w, short &h,
                                int lFlag, int rFlag, int tFlag, int bFlag);
template short rspLasso<U32>(U32 ignoreColor, RImage* pimSrc, short &x, short &y, short &w, short &h,
                             int lFlag, int rFlag, int tFlag, int bFlag);
...
void instantiateLasso()
{
   ...
   rspLasso( (U32)i, pim, i, i, i, i, 1, 1, 1, 1);
                 ...
                 short sW, short sH, const RRect* prDst, const RRect* prSrc)
{
   ...
   const S32 clChosenAlignment = 4; // in bytes
                ...
                short sDstY, short sW, short sH, const RRect* prDst, const RRect* prSrc)
{
   ...
   if (pimSrc->m_type == RImage::IMAGE_STUB) sBlitTypeSrc = (short)((intptr_t)pimSrc->m_pSpecial);
   if (pimDst->m_type == RImage::IMAGE_STUB) sBlitTypeDst = (short)((intptr_t)pimDst->m_pSpecial);
...
inline void _ClearRect(WORDSIZE color, WORDSIZE* pDst, S32 lDstPitch,
                       short sHeight, short sByteWidth)
{
   union
   {
...
//RECT_DONTLOCK:
   ...
   {
      // 32-bit copy
      U32 ulColor = color; //TODO: was U32
