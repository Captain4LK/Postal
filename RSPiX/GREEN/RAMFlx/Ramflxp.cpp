...
//
// Call any of these to get a UCHAR, USHORT, or U32 from the memory area.  The
...
//                  bVal = MEM_BYTE(m_pCurFlxBuf);   
         ...
         pbRow += (U32)pimageRead->lPitch;
         }
      #else
         // Locals needed (b/c we can't use this->).
         S32      lPitch      = pimageRead->lPitch;
         ...
         __asm
            ...
            BRunPacketLoopDone:
               ...
               mov   edi,   pbRow
               add   edi,   lPitch            ; pbRow += (U32)pimageRead->lPitch;
            ...
            pfile->Seek((S32)(pCurFlxBuf - pfile->GetMemory()), SEEK_SET);
...
//   y = MEM_WORD(m_pCurFlxBuf);   
   ...
   pbRow = (UCHAR*)pimageRead->pData + ((U32)y * (U32)pimageRead->lPitch);
...
//               bVal = MEM_BYTE(m_pCurFlxBuf);   
      ...
      __asm
         ...
         LCPacketLoop:                     ; while (packets > 0)
            ...
            add   edi, ebx                  ; pbPix = pbPix + (U32)bVal;
...
//               wVal = MEM_WORD(m_pCurFlxBuf);   
      ...
      pbPix = (UCHAR*)pimageRead->pData + ((U32)y * (U32)pimageRead->lPitch);
...
//               memset(pbPix, (int)wVal, (size_t)sCount);
      ...
      if (bLastByte == TRUE)
         ...
         pbPix = (UCHAR*)pimageRead->pData + (((U32)y + 1L) * (U32)pimageRead->lPitch) - 1L;
   ...
   SS2PacketLoop:
      ...
      add   edi, edx                  ; pbPix = pbPix + (U32)bVal;
...
short CRamFlx::ReadHeader(CNFile* pfile)
   ...
   short sError=0;
    S32 lSizeFile = m_filehdr.lEntireFileSize - m_filehdr.lOffsetFrame1;
...
short CRamFlx::AllocBuf(CImage* pimage, S32 lWidth, S32 lHeight, short sColors)
   {
   short sError=0;
...
void CRamFlx::CopyBuf(CImage* pimageDst, CImage* pimageSrc)
   ...
   for (short y = 0; y < pimageSrc->lHeight; y++)
      ...
      pbSrc += (U32)pimageSrc->lPitch;
      pbDst += (U32)pimageDst->lPitch;
      }                                          
...
short CRamFlx::CreateFramePointers(void)
   ...
   if ((m_plFrames = (S32*)malloc((m_filehdr.sNumFrames+1) * sizeof(S32))) == NULL)
      sError = -1;
   else
      {
...
//         m_pCurFlxBuf = MEM_SEEK(m_pCurFlxBuf,lSizeFrame - sizeof(U32)); 
         m_file.Seek(lSizeFrame - sizeof(U32), SEEK_CUR);
         }
      }
