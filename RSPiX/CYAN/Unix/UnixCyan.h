...
typedef struct
{
   ...
   S32 lParam;       // Used as CursHandle or CCrsrHandle but declared as S32
...
extern void rspSetQuitStatusFlags(
   S32 lModifiers);                    // In:  Modifier key flags (RSP_GKF_*)
...
extern short rspSetPrinterResolution(     // Returns 0 if successfull, non-zero otherwise
   S32 lReqHorzRes,                       // In:  Requested horizontal resolution (dpi)
   S32 lReqVertRes,                       // In:  Requested vertical resolution (dpi)
...
extern short rspGetPrinterResolution(     // Returns 0 if successfull, non-zero otherwise
   S32* plHorzRes,                        // Out: Horizontal resolution (dpi)
   S32* plVertRes);                       // Out: Vertical resolution (dpi)

extern short rspGetPrinterPageSize(       // Returns 0 if successfull, non-zero otherwise
   S32* plPageWidth,                   // Out: Page width (in dots)
   S32* plPageHeight);                    // Out: Page height (in dots)
...
extern short rspGetPrinterPageMargins(    // Returns 0 if successfull, non-zero otherwise
   S32* plLeftMargin,                     // Out: Page's left margin (in dots)
   S32* plRightMargin,                    // Out: Page's right margin (in dots)
   S32* plTopMargin,                   // Out: Page's top margin (in dots)
   S32* plBottomMargin);                  // Out: Page's bottom margin (in dots)
...
extern short rspPrintToPage(              // Returns 0 if successfull, non-zero otherwise
   ...
   S32 lPitch,                         // In:  Source pitch
   S32 lWidth,                         // In:  Source width
   S32 lHeight,                           // In:  Source height
   short sDepth,                          // In:  Source depth (1 or 8)
   S32 lSrcX,                             // In:  Source (image) X coord
   S32 lSrcY,                             // In:  Source (image) Y coord
   S32 lSrcW,                             // In:  Source (image) width
   S32 lSrcH,                             // In:  Source (image) height
   S32 lDstX,                             // In:  Destination (page) X coord
   S32 lDstY,                             // In:  Destination (page) Y coord
   S32 lDstW,                             // In:  Destination (page) width
   S32 lDstH);                         // In:  Distination (page) height
...
extern short rspPrintToPage(              // Returns 0 if successfull, non-zero otherwise
   ...
   S32 lSrcPalIncBytes,                // In:  What to add to pointers to move to next value
   S32 lSrcX,                             // In:  Source (image) X coord
   S32 lSrcY,                             // In:  Source (image) Y coord
   S32 lSrcW,                             // In:  Source (image) width
   S32 lSrcH,                             // In:  Source (image) height
